...
const motorMountGearMotor = (motorData, axles, innerBodySize, wallsThickness) => {
  ...
  .map(motorMountData => {
    ...
    const motorBlockerShape = translate([blockerThickness / 2, -(cutDepth - 1.5), 0],
      rotate([90, 0, -90], linear_extrude({height: blockerThickness}, motorBlockerOutline))
    )

    const motorSidePlate = translate([-sidePlatesWidth - motorWidth / 2, centralSpacing / 2, 0],
      union(
